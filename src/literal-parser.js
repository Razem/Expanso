  ...
  members: {
    ...
    putId: function (type, start, end) {
      ...
      if (type === "regexp-expanso") {
        ...
        var ch = lit[0];
        lit = lit.replace(/#\{@(\w|\$)([^\}]+)\}/g, function (math, p1, p2) { return ch + " + this." + p1 + p2.replace(/\\"/g, '"') + " + " + ch; })
                 .replace(/#\{@([^\}]+)\}/g, function (math, p1) { return ch + " + this" + p1.replace(/\\"/g, '"') + " + " + ch; })
                 .replace(/#\{([^\}]+)\}/g, function (math, p1) { return ch + " + " + p1.replace(/\\"/g, '"') + " + " + ch; });
      }
